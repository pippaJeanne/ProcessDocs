...
#response = client.models.generate_content(
 #   model='gemini-2.5-flash', contents=f"En prenant le modèle d'encodage TEI dans {template}, prends le texte océrisé dans {text} et fais l'encodage TEI du text océrisé en suivant (strictement) le modèle fourni. Tu peux aussi prendre le contenu de {wikiLinks} comme référence pour les entités nommées. Produis le contenu dans un format XML-TEI.")
